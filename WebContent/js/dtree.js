...
// Tree object
//-
function dTree(objName, pathImgCss, formId,pathImgCssDir) {
	this.config = {
		target					: null,
		folderLinks			: true,
		useSelection		: true,
...
function isCheckedByRec( form ,regx){
	...
	return false;
}/**set default checked*/ dTree.prototype.setDefaultCheck=function(nodeIds){	var ids = document.getElementsByName("id");	var tempIds = nodeIds.split(",");	for(var n=0; n<tempIds.length; n++){ //loop default list		for(var i=0; i<ids.length; i++){			if(ids[i].value == tempIds[n]){				ids[i].checked = true;				break;			}		}	}};
